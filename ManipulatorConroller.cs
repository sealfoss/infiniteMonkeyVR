...
public class ManipulatorConroller : MonoBehaviour
{
    ...
    void BuildJoint()
    {
        grabbedObj.gameObject.AddComponent<FixedJoint>();
        joint = grabbedObj.GetComponent<FixedJoint>();
    ...
    public void Drop()
    {
        if (grabbedObj.jointedMove)
        {
            //first, impart the manipulator's velocity and angular velocity upon the object
            grabbedObj.rigidBody.velocity = this.rigidbody.velocity;
            grabbedObj.rigidBody.angularVelocity = this.rigidbody.angularVelocity;
            //now, destroy the joint along with the manipulator's rigidbody (we won't be using it anymore), and reset grab attributes
            joint = null;
            Destroy(grabbedObj.GetComponent<ConfigurableJoint>());
            rigidbody = null;
